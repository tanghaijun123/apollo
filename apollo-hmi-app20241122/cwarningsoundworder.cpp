...
//    }
    ...
    if(iSoundType < 0){
        ...
        m_pMediaPlayer->play();
        quint32 interval[3] = {8000, 8000, 21000};
        startReplayTmr(interval[m_iCurSoundType]);
    }
}
...
void CWarningSoundWorker::onPlayKeySound()
{
    ...
    if(!m_pMediaPlayerKey){
        ...
        QString strSoundName = QString("%1%2").arg(m_strSoundPath, m_strKeySoundFileName);
        if(QFile::exists(strSoundName)){
            m_pMediaPlayerKey->setMedia(QUrl::fromLocalFile(strSoundName));
        }
    }
...
void CWarningSoundWorker::timerEvent(QTimerEvent *event)
{
    if(event->timerId() == m_iDelayPlaySound){
        onDelayPlayTimeOut();
        return;
    } else if(event->timerId() == m_timerToReplay) {
        onReplayTimeout();
        return;
    }
...
void CWarningSoundWorker::startReplayTmr(int interval)
{
    stopDelayPlayTmr();

    if(interval > 0) {
        m_timerToReplay = startTimer(interval);
    }
}

void CWarningSoundWorker::stopReplayTmr()
{
    if(m_timerToReplay > 0){
        killTimer(m_timerToReplay);
    }
    m_timerToReplay = 0;
}
